...
{
	printf("bType = %d, bMode = %d, key_len = %d\n", bType, bMode, key_len);
	int i,j,k;
	...
	else if (bMode == CBC)
	{
		...
		else if (nKey == 3)
		{
			...
			for (int ii = 0; ii < j * 8; ++ii)
			{
				printf("%02x ", *(unsigned char *)(Out + ii));
			}
			printf("\n\n\n");
		}
		else
...
/*******************************************************************
  ...
  参 数 说 明：	nType	:填充类型：PAD类型
				In		:输入数据指针
				in_len	:输入数据的长度
				Out		:输出数据指针
				pad_len	:填充后的长度

  返回值 说明：	char	:是否填充成功，1：成功，0：失败
  作       者:	huangjf
  修 改 历 史：

  更 新 日 期：	2009.12.7
  算 法 说 明： 内容采用PKCS5Padding方式填充，即长度以8字节切分，不能被8整除的末尾部分，根据长度不足8字节的部分，填充“0x01”—“0x08”，
                如不足1字节，则填充1个“0x01”，如不足2字节，则填充2个“0x02”，以此类推，
			    如整除，则填充8个“0x08”
*******************************************************************/
int	RunPad(int nType, const char *In, unsigned in_len, char *Out, int *pad_len)
{
	int res = (in_len & 0x00000007);

	*pad_len	= in_len + 8 - res;
	...
	case PAD_PKCS_5:
		{
			memset(Out+in_len, 0x08, 8-res);
			for (size_t i=0; (res!=0) && (i<(8-res)); i++)
			{
				memset(Out+in_len+i, i+1, 1);
			}
		}
	case PAD_PKCS_7:
		{
...
/*******************************************************************
  函 数 名 称:	RunRsm
  功 能 描 述：	解密后,将填充的字符去掉
  参 数 说 明：	szText	:解密后字符串
