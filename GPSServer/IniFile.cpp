...
BOOL CIniFile::OpenIniFile (CCHR * FileName)
{
	...
	while (fgets (Str, MAX_LINE_LEN, m_pIniFile) != NULL)
	{
		if ((pStr = strchr (Str, 0x0d)) != NULL)
		{
			*pStr = 0;
		}
		if ((pStr = strchr (Str, 0x0a)) != NULL)
		{
			*pStr = 0;
		}
...
int main (void)
{
	...
	c = getchar();
	#endif
	iFile.DeleteKey    ("Test" , "Name");
	iFile.WriteString("Test", "Name", "ouzhx");
	iFile.WriteIniFile ("Test.Ini");
